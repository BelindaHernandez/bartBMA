...
GS_ITEs_PIs_bartBMA<-function(num_iter,burnin,l_quant,u_quant,newdata=NULL,update_resids=1,
  ...
  if(update_resids==0){
    if(is.null(newdata) && length(trained_bart_BMA)==16){
      ...
      gs_chains<-gibbs_sampler_ITE_no_update(trained_bart_BMA$sumoftrees,trained_bart_BMA$obs_to_termNodesMatrix,trained_bart_BMA$response,trained_bart_BMA$bic,num_iter, burnin,trained_bart_BMA$nrowTrain,
                                   nrow(trained_bart_BMA$test_data),trained_bart_BMA$a,trained_bart_BMA$sigma,0,
                                   trained_bart_BMA$nu,trained_bart_BMA$lambda,trained_bart_BMA$sum_residuals,
      ...
      gs_chains<-gibbs_sampler_ITE_no_update2(trained_bart_BMA$sumoftrees,trained_bart_BMA$obs_to_termNodesMatrix,trained_bart_BMA$response,trained_bart_BMA$bic,num_iter, burnin,trained_bart_BMA$nrowTrain,
                                trained_bart_BMA$a,trained_bart_BMA$sigma,0,trained_bart_BMA$nu,trained_bart_BMA$lambda,trained_bart_BMA$sum_residuals,
                                all_treated_data,all_control_data)
      ...
      gs_chains<-gibbs_sampler_ITE_no_update(trained_bart_BMA$sumoftrees,trained_bart_BMA$obs_to_termNodesMatrix,trained_bart_BMA$response,trained_bart_BMA$bic,num_iter, burnin,trained_bart_BMA$nrowTrain,
                                   nrow(newdata), trained_bart_BMA$a,trained_bart_BMA$sigma,0,trained_bart_BMA$nu,trained_bart_BMA$lambda,trained_bart_BMA$sum_residuals,
                                   all_treated_data,all_control_data)
    }
  }else{
    if(is.null(newdata) && length(trained_bart_BMA)==16){
      ...
      all_control_data <- cbind(rep(0,nrow(trained_bart_BMA$test_data)), trained_bart_BMA$test_data)      
      #if test data specified separately
      gs_chains<-gibbs_sampler_ITE(trained_bart_BMA$sumoftrees,trained_bart_BMA$obs_to_termNodesMatrix,trained_bart_BMA$response,trained_bart_BMA$bic,num_iter, burnin,trained_bart_BMA$nrowTrain,
                               nrow(trained_bart_BMA$test_data),trained_bart_BMA$a,trained_bart_BMA$sigma,0,
                               trained_bart_BMA$nu,trained_bart_BMA$lambda,trained_bart_BMA$sum_residuals,
                               all_treated_data,all_control_data)
    }else if(is.null(newdata) && length(trained_bart_BMA)==14){
      ...
      all_control_data <- cbind(rep(0,nrow(x_covariates)), x_covariates)
      #else return Pred Ints for training data
      gs_chains<-gibbs_sampler_ITE2(trained_bart_BMA$sumoftrees,trained_bart_BMA$obs_to_termNodesMatrix,trained_bart_BMA$response,trained_bart_BMA$bic,num_iter, burnin,trained_bart_BMA$nrowTrain,
                                trained_bart_BMA$a,trained_bart_BMA$sigma,0,trained_bart_BMA$nu,trained_bart_BMA$lambda,trained_bart_BMA$sum_residuals,
                                all_treated_data,all_control_data)
