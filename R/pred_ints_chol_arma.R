...
pred_ints_chol_arma <-function(object,num_iter,burnin,l_quant,u_quant,newdata=NULL,update_resids=1){
  ...
  if(is.null(newdata) && length(object)==16){
    ...
    ret<-pred_ints_chol_attempt_outsamp(object$sumoftrees,object$obs_to_termNodesMatrix,object$response,object$bic,num_iter, burnin,object$nrowTrain,
                                   nrow(object$test_data),object$a,object$sigma,0,object$nu,
                                   object$lambda,#diff_inital_resids,
                                   object$test_data,l_quant,u_quant
    )
