...
pred_ints_from_rmixt <-function(object,num_iter,burnin,l_quant,u_quant,newdata=NULL,update_resids=1,num_cores=1){
  ...
  if(is.null(newdata) && length(object)==16){
    ...
    inputs_for_draws<-mean_vars_lin_alg_outsamp(object$sumoftrees,object$obs_to_termNodesMatrix,object$response,object$bic,num_iter, burnin,object$nrowTrain,
