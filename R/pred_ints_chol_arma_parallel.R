...
pred_ints_chol_arma_parallel <-function(object,num_iter,burnin,l_quant,u_quant,newdata=NULL,update_resids=1,num_cores=1){
  ...
  if(is.null(newdata) && length(object)==16){
    ...
    ret<-pred_ints_chol_parallel_outsamp(object$sumoftrees,object$obs_to_termNodesMatrix,object$response,object$bic,num_iter, burnin,object$nrowTrain,
